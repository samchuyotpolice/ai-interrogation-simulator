...
const PageLayout: React.FC<PageLayoutProps> = ({ children, currentUserRole, onLogout, theme, toggleTheme }) => {
  const getRoleText = (role: UserRole | null) => {
    ...
    if (role === UserRole.SYSTEM_ADMIN) return UI_TEXT.roleSystemAdmin;
    return '';
  }
  ...
  return (
    <div className={`min-h-screen flex flex-col ${theme === 'light' ? 'light-theme' : ''}`} dir="rtl">
          ...
          <div className="flex items-center space-x-3 rtl:space-x-reverse">
            <img
              ...
              onError={(e) => {
                ...
                placeholder.textContent = (UI_TEXT && UI_TEXT.appName) ? UI_TEXT.appName.substring(0,1) : "S";
                target.parentNode?.insertBefore(placeholder, target);
              }}
            />
            <h1 className={`text-xl sm:text-2xl font-bold ${appNameColor}`}>{ (UI_TEXT && UI_TEXT.appName) ? UI_TEXT.appName : "Interrogation Simulator"}</h1>
          </div>
      ...
      <footer className={`text-center p-4 text-xs sm:text-sm ${footerBgColor} ${footerTextColor}`}>
        © {new Date().getFullYear()} { (UI_TEXT && UI_TEXT.appName) ? UI_TEXT.appName : "Interrogation Simulator"}. כל הזכויות שמורות.
      </footer>
    </div>
  );
};
