// import * as React from 'react'; // No longer needed here for JSX augmentation
import React from 'react'; // Added for JSX augmentation
import { Chat, Content, Part } from "@google/genai"; // Removed GenerativeContentBlob, Added Part
import type { GdmLiveAudio } from './live-audio'; 
import type { GdmLiveAudioVisuals3D } from './live-audio-visuals-3d'; 
...
export interface UserCommandPayloadBase {
    targetSessionId?: string; // To specify which session this command is for (future use with live sessions)
}
...
export type GeminiPart = Part; // Updated to use Part
// Removed GeminiGenerativeContentBlob as GenerativeContentBlob is not exported and Part covers inlineData


// Define specific prop interfaces for custom elements
export interface GdmLiveAudioElementProps {
  initialSystemPrompt?: string;
}

export interface GdmLiveAudioVisuals3DElementProps {
...
// Window augmentations remain here
declare global {
    interface Window {
...
// Augment React's JSX types for custom elements
declare module 'react' {
    namespace JSX {
        interface IntrinsicElements {
            'gdm-live-audio': React.DetailedHTMLProps<
                React.HTMLAttributes<GdmLiveAudio> & GdmLiveAudioElementProps,
                GdmLiveAudio
            >;
            'gdm-live-audio-visuals-3d': React.DetailedHTMLProps<
                React.HTMLAttributes<GdmLiveAudioVisuals3D> & GdmLiveAudioVisuals3DElementProps,
...
export type LoadedAIAgent = Omit<AIAgent, 'isDefault' | 'isEditable' | 'personalityTraits' | 'agentType' | 'conversationStarters' | 'recommendedModel' | 'capabilities'> & {
    isDefault: boolean;
    isEditable: boolean;
    personalityTraits: string[]; 
    agentType: AIAgentType; 
...
// Note: The trailing 'export {};' was removed as it's redundant given other exports.
